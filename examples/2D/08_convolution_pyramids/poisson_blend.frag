    ...
    if (!u_convolutionPyramidUpscaling) {
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                ...
                // vec3 src = (texture2D(u_buffer2, saturate(uv)).rgb - 0.5) * g[ absi(dx) ] * g[ absi(dy) ];
                // color.rgb += src * msk;
            }
        }
        ...
        for (int dy = -2; dy <= 2; dy++) {
            for (int dx = -2; dx <= 2; dx++) {
                ...
                vec3 src = texture2D(u_buffer2, saturate(st + offset)).rgb - 0.5;
                color.rgb += src * msk * .05;
            }
        }
