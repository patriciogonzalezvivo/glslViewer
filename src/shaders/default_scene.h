...
#if defined(LIGHT_SHADOWMAP) && defined(LIGHT_SHADOWMAP_SIZE)
float textureShadowLerp() {
    return textureShadowLerp(v_lightCoord.xyz);
}
#endif
...
#if defined(LIGHT_SHADOWMAP) && defined(LIGHT_SHADOWMAP_SIZE)
float textureShadowPCF() {
    return textureShadowPCF(v_lightCoord.xyz);
}
#endif
...
void lightWithShadow(vec3 _diffuseColor, vec3 _specularColor, vec3 _N, vec3 _V, float _NoV, float _roughness, float _f0, out vec3 _diffuse, out vec3 _specular) {
    ...
    _diffuse += lightDiffuse * shadow;
    _specular += saturate(lightSpecular) * shadow;
}
